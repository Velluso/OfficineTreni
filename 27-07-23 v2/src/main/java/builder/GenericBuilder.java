...
public class GenericBuilder extends TrenoBuilder {
	...
	public Treno costruisciTreno(String sigla,String compagnia) throws VagoneNonRiconosciutoException,VagoniIncompatibiliException,LocomotivaException, PosizioneRistoranteException,PesoTrainatoException {
		...
		for(int i=0;i<caratteri.length;i++) {
			...
			if(compagnia.equalsIgnoreCase("italo")) {
				switch(caratteri[i]) {
					case 'h':	
			...
			}else if(compagnia.equalsIgnoreCase("freccia rossa")) {
				switch(caratteri[i]) {
				case 'h':	
			...
			}else if(compagnia.equalsIgnoreCase("trenord")) {
				switch(caratteri[i]) {
				case 'h':	
